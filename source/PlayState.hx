...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBar.createFilledBar(dad.colorHBar, 0xFF66FF33);
		// healthBar
		add(healthBar);
		...
		pauseButton = new FlxUIButton();
		pauseButton.loadGraphic(Paths.image('pauseButton'));
		pauseButton.cameras = [camHUD];
		add(pauseButton);

		if (FlxG.save.data.downscroll){
			pauseButton.setPosition(FlxG.width-130, 31);
		} else {
			switch (mcontrols.mode)
			{
				case VIRTUALPAD_LEFT | HITBOX | KEYBOARD:
					pauseButton.setPosition(FlxG.width-130, FlxG.height-130);
				case VIRTUALPAD_RIGHT | VIRTUALPAD_CUSTOM:
					pauseButton.setPosition(50, FlxG.height-130);
				// default: 
				// 	pauseButton.setPosition(FlxG.width-130, FlxG.height-130);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		var pauseTrig = FlxG.keys.justPressed.ENTER 
					|| FlxG.keys.justPressed.ESCAPE 
					#if android 
					|| pauseButton.justPressed
					|| FlxG.android.justReleased.BACK 
					#end ;

		if (pauseTrig && startedCountdown && canPause)
		{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.validScore)
					{
						#if newgrounds NGio.unlockMedal(60961); #end
